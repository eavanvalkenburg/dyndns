...
from azure.mgmt.dns.v2018_05_01.models import ARecord, AaaaRecord, RecordSet, RecordType
...
async def update_dns(req: func.HttpRequest) -> func.HttpResponse:
    ...
    logging.info("Requested url: %s", req.url)
    auth = req.headers.get(HEADER_AUTH)
    if auth is None:
        logging.warning("Unauthorized request, no auth header")
    ...
    if auth != stored_auth:
        logging.warning("Unauthorized request, incorrect credentials")
        return func.HttpResponse(UNAUTHORIZED)
    ...
    if not hostname or not (ip_address or ipv6_address):
        logging.warning("Missing hostname or ip address")
...
async def update_dns_records(
    ...
    logging.info(
        "Updating dns records for %s with ips: %s, %s",
        hostnames,
        ipv4_address,
        ipv6_address,
    )
...
async def run_check_and_update(
    ...
    try:
        ...
        raise ZoneException("Zone not found") from exc
...
async def record_to_update(
    ...
    try:
        ...
        logging.info("Record not found, creating")
        return True
